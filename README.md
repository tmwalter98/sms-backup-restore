...
```bash
sms-backup-restore$ sam build --use-container
```
...
```bash
sms-backup-restore$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sms-backup-restore$ sam local start-api
sms-backup-restore$ curl http://localhost:3000/
```
...
```bash
sms-backup-restore$ sam logs -n HelloWorldFunction --stack-name sms-backup-restore --tail
```
...
```bash
sms-backup-restore$ pip install -r tests/requirements.txt --user
# unit test
sms-backup-restore$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sms-backup-restore$ AWS_SAM_STACK_NAME="sms-backup-restore" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "sms-backup-restore"
```
